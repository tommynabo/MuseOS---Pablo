...
router.post('/workflow/parasite', requireAuth, async (req, res) => {
    ...
    try {
        // Step 1: Get creators
        const { data: creators, error: creatorError } = await supabase.from('creators_pablo').select('linkedin_url');
        ...
        for (const post of highEngagementPosts) {
            ...
            // Save to DB
            await supabase.from('posts_pablo').insert({
                user_id: user.id,
...
router.patch('/posts/:id', requireAuth, async (req, res) => {
    ...
    const { data, error } = await supabase.from('posts_pablo')
        .update({ status })
        .eq('id', id)
        .select()
        .single();
...
router.post('/workflow/generate', requireAuth, async (req, res) => {
    ...
    try {
        ...
        for (const post of highEngagementPosts) {
            ...
            // Save to DB
            const { error: insertError } = await supabase.from('posts_pablo').insert({
                user_id: user.id,
                original_post_id: post.id || 'unknown',
                ...
                meta: {
                    ...
                    original_url: post.linkedinUrl || post.url || post.postUrl || null,
                    engagement: {
                        likes: getMetric(post, 'likes'),
                        comments: getMetric(post, 'comments'),
                        shares: getMetric(post, 'shares')
